...
# Sample SWOT data generator
def generate_sample_swot(company_name: str) -> Dict[str, Any]:
    """Generate sample SWOT data for testing"""
    strengths = [
        f"Market leader in {company_name.lower()} industry with strong brand recognition",
        "Vertically integrated supply chain and in-house production",
        "Advanced technology and continuous product updates",
        "Strong distribution network providing competitive advantage",
    ]
    
    weaknesses = [
        "Production quality inconsistencies and service capacity",
        "Heavy reliance on key personnel and public persona",
        "Limited product variety compared to competitors",
        "High prices limiting mass-market accessibility",
    ]
    
    opportunities = [
        f"Expanding global market for {company_name.lower()} products",
        "New business segments and market growth potential",
        "Emerging technologies and service innovations",
    ...
    # Generate a realistic SWOT report
    report = f"""
...
""" + "\n- ".join(threats)
    ...
    critique = f"The analysis provides comprehensive coverage of {company_name}'s strategic position. Strengths and opportunities are well-articulated with specific examples. Recommend adding more quantitative data points for market share and financial metrics. Overall quality meets professional standards."
    
    return {
        "company": company_name,
        "score": round(random.uniform(7.0, 9.5), 1),
        "revision_count": random.randint(1, 3),
...
async def process_workflow(workflow_id: str):
    ...
    try:
        ...
        if workflow_data["current_step"] == "initializing":
            ...
            swot_data = generate_sample_swot(state["company_name"])
            
            # Update state with results
            state.update({
                "draft_report": swot_data["draft_report"],
                "critique": swot_data["critique"],
                "score": swot_data["score"],
                "revision_count": swot_data["revision_count"]
            })
